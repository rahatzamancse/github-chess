...
if CHESS_GAME_CMD == 'move'
    ...
    issues&.each do |issue|
        ...
        game_stats[:moves] += 1
        if ENV.fetch('REPOSITORY') == 'rahatzamancse/github-chess'
    ...
    @octokit.add_comment(ENV.fetch('REPOSITORY'), ENV.fetch('EVENT_ISSUE_NUMBER'), "That's game over! Thank you for playing that chess game. That game had #{game_stats[:moves]} moves, #{game_stats[:players]&.length} players, and went for #{hours} hours. Let's play again at https://github.com/rahatzamancse/github-chess.\n\nPlayers that game: #{game_stats[:players].join(', ')}")
    end
end



@octokit.add_comment(ENV.fetch('REPOSITORY'), ENV.fetch('EVENT_ISSUE_NUMBER'), "@#{ENV.fetch('EVENT_USER_LOGIN')} Done. View back at https://github.com/rahatzamancse/github-chess\n\nAsk a friend to take the next move: [Share on Twitter...](https://twitter.com/share?text=I'm+playing+chess+on+a+GitHub+Profile+Readme!+I+just+moved.+You+have+the+next+move+at+https://github.com/rahatzamancse/github-chess)")          
@octokit.close_issue(ENV.fetch('REPOSITORY'), ENV.fetch('EVENT_ISSUE_NUMBER'))



#
# Update rahatzamancse/github-chess/README.md
# ---------------------------------------
...
Raise an [issue](https://github.com/#{ENV.fetch('REPOSITORY')}/issues), and include the text _cc @rahatzamancse_.
